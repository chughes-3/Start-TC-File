...
#region Deal with Migration errors
            ...
            {// reg entry no old file
                if (File.Exists(tpdataPath))
                {
                    DialogResult mbResult = MessageBox.Show("There is a migration data flag set, but no old TPDATA file, Delete the Migration Flag?", thisProginst.mbCaption, MessageBoxButtons.OKCancel, MessageBoxIcon.Error); //Delete reg entries and return on OK, exit on cancel
                    if (mbResult == DialogResult.OK)
                    {
                        DeleteMigrationRegEntries();
                        return;
                ...
                {
                    DialogResult mbResult = MessageBox.Show("There is a migration data flag set, but no old TPDATA file, Delete the Migration Flag?", thisProginst.mbCaption, MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                    if (mbResult == DialogResult.OK)
                    {
                        DeleteMigrationRegEntries();
                        StartTAUtility("");
        ...
        private void ShareCreate(string ShareName, string FolderPath, string Description)
        {
            ...
            if ((uint)(outParams.Properties["ReturnValue"].Value) != 0)
            {
                ...
                MessageBox.Show("Unable to share P Drive. The error message was " + errCode);
                Environment.Exit(1);
            }
        }
